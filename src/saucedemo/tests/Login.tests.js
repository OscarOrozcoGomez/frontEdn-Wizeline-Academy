...
//     await t.expect(LoginPage.loginErrorText.textContent).eql(dataSet.ErrorMessages.UserDontMatchMessage)
// });
...
test("Validate error message when only username is introduced", async t => {
    ...
    await t.expect(LoginPage.loginErrorText.textContent).eql(dataSet.ErrorMessages.PasswordRequiredMessage)
});

test("Validate error message when only password is introduced", async t => {
    await LoginPage.populatePasswordField(PASSWORD);
    await LoginPage.clickOnLoginButton();
    await t.expect(LoginPage.loginErrorText.textContent).eql(dataSet.ErrorMessages.UserRequiredMessage)
});
